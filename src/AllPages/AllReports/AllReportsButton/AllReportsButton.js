...
const AllReportsButton = () => {
  ...
  const handelPrapereReport = (e, idTocheck) => {
    setActiveId(idTocheck)
    if (e === "Prepare Report") {
  ...
  // ************************featch data to print****************************
  const AllReportsData = useSelector((state) => state.newAllEntries)
  console.log(AllReportsData)

  function findBookingById(id) {
    for (const order of AllReportsData.orders) {
  ...
  const handelPrintReport = async (e) => {
    // Call the function with the specified ID
    const originalData = findBookingById(e);
      ...
      {data.testNames.map((name, testNameIndex) => (
        <ReactTooltip key={`tooltip-${dataIndex}-${testNameIndex}`} id={`tooltip-${dataIndex}-${testNameIndex}`}>
          {name.name}
                  ...
                  <div onClick={() => handelPrapereReport(data.prepareReport, data.idToCheck)} className={styles.allDataLayoutTopPatientDetails}>
                    <div className={styles.allDataLayoutTopTextPrepareReport}>{data.prepareReport}</div>

                  </div>
                  <div className={styles.allDataLayoutTopPrintBox}>
                    <div onClick={() => handelPrintReport(data.idToCheck)} className={styles.allDataLayoutTopTextIcon}>
            ...
            prepareReportPopUp ?
              <div className={styles.modalOverlay}>
                <div className={styles.detailsAndTestLayout}>
                  ...
                  <PatientDetails onClick={handlePrepareReportPopUpClick} id={activeId} />
                </div>
              </div> : ""
          }
          {
            editReportPopUp ?
              <div className={styles.modalOverlay}>
                <div className={styles.detailsAndTestLayout}>
                  ...
                  <RejectedReportPopUp onClick={handleEditReportPopUpClick} id={activeId} />
                </div>
              </div> : ""
          }
          {
